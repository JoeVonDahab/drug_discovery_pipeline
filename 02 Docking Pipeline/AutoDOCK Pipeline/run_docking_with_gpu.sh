...
DEVICE_NUMBER="2"          # !!! SET THIS to the GPU device number (e.g., 1 for the first GPU, 2 for the second) !!!
...
echo "  Using GPU Device Number: $DEVICE_NUMBER" # Added this line for feedback
echo "-------------------------------------------"
...
for ligand_path in "$LIGAND_DIR"/*.pdbqt; do
    ...
    # Run AutoDock-GPU with proper -resnam quoting and specified device number
    "$AUTODOCK_GPU_EXEC" \
        --ffile "$map_file_abs" \
        --lfile "$ligand_abs" \
        --nrun "$NUM_RUNS" \
        -resnam "$output_base" \
        --devnum "$DEVICE_NUMBER"   # <-- Added GPU device selection
