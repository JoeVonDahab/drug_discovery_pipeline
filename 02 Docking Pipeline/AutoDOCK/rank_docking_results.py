...
def main():
    ...
    copied = 0
    # Filter for copying: only ligands where PDBQT exists ('Within_Pocket' is True)
    # The list 'processed_rows' is already sorted by affinity.
    ligands_to_copy_candidates = [
        data_dict for data_dict in processed_rows if data_dict['Within_Pocket']
    ]
    
    # Select top N from these candidates for copying
    top_ligands_to_copy = ligands_to_copy_candidates[:TOP_N]

    for data_dict in top_ligands_to_copy:
        name = data_dict['Compound']
        # Source PDBQT path was implicitly checked when 'Within_Pocket' was set.
        # If 'Within_Pocket' is True, this file should exist.
        src = INPUT_PDBQT_DIR / pdbqt_name(name)

        shutil.copy2(src, TOP_OUT_DIR / src.name)
        copied += 1
        
    print(f"Copied {copied} PDBQT files (top {TOP_N} with existing PDBQTs) to {TOP_OUT_DIR}")
